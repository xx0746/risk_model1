...
class Write_Oracle(object):
    ...
    def get_max_index(self, table_name):
        ...
        s_id = Read_Oracle().read_oracle(sql=read_sql, database='dbdm')
        return s_id['ID'].values[0]
    ...
    def change_lastupdate(self, old_table_name, org_code, alarm):
        ...
        if org_code is not None and alarm is None:
            try:
                ...
                logger.error(e)
        elif org_code is not None and len(alarm) == 2:
            model_code = alarm[0]
    ...
    @logger.catch()
    def write_oracle(self, table_name, new_df, org_code, alarm, iscurrent=True):
        """
        风控产品封装的写入接口
