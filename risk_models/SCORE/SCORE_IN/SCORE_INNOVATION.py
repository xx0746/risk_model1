...
class ScoreIn(object):
    ...
    def score_innovation(self):
        ...
        a.THREE_FLOOR,
        a.FOURTH_FLOOR,
        COUNT(DISTINCT DEAL_WITH_NO) AS EVENT_NUMBER 
        FROM DEAL_WITH a 
        WHERE a.SECOND_FLOOR = 'GNYTJKYJ' AND a.STATUS in ('48','49')
        GROUP BY a.BIZOP_ETPS_SCCD,
        a.BIZOP_ETPS_NO,
        a.THREE_FLOOR,
        a.FOURTH_FLOOR
        """
        in_result = Read_Oracle().read_oracle(sql=sql_text, database='dblgsa')
...
if __name__ == '__main__':
    ...
    score_in = ScoreIn(org_code = org_code, base_time = base_time, child_task_id = child_task_id)
    score_in.run_score_innovation()