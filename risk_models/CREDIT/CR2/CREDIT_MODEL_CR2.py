...
class CreditModelCr2(object):
    ...
    def model_cr2(self):
        p1=f"""select * from {TableList.BD_RISK_DETAIL_CREDIT_CR2.value} WHERE ISCURRENT = 1 AND ORG_CODE LIKE '{self.org_code}' and CUSTOMER_CODE = 'FTA_LG' """
        ...
        t2['RISK_LABEL'] = '投资人涉诉未结案'
        t2['SCORE'] = self.score_crit[4]

        t1.loc[(t1['EVENT_TYPE']=='涉诉一审')&(t1['ROLE'] == '被告'),'RISK_LABEL'] = '投资人涉诉一审，为被告'
        t1.loc[(t1['EVENT_TYPE']=='涉诉一审')&(t1['ROLE'] == '被告'),'SCORE'] = self.score_crit[3]
        t1.loc[(t1['EVENT_TYPE']=='涉诉再审')&(t1['ROLE'] == '被告'),'RISK_LABEL'] = '投资人涉诉二审，为被告'
        t1.loc[(t1['EVENT_TYPE']=='涉诉再审')&(t1['ROLE'] == '被告'),'SCORE'] = self.score_crit[3]
        t1.loc[t1['ROLE'] == '原告','RISK_LABEL'] = '投资人涉诉，为原告'
        t1.loc[t1['ROLE'] == '原告','SCORE'] = self.score_crit[1]
        t1.loc[t1['ROLE'] == '第三人','RISK_LABEL'] = '投资人涉诉，为第三人'
        t1.loc[t1['ROLE'] == '第三人','SCORE'] = self.score_crit[2]
        t1.loc[t1['EVENT_TYPE'] == '失信被执行','RISK_LABEL'] = '投资人为失信被执行人'
        ...
        if ((df_result.shape[0] == 1) & ((pd.isnull(df_result.loc[0,'CASE_ID'])|(df_result.loc[0,'CASE_ID'] == 'nan')))):
            df_result.loc[0,'RISK_LABEL'] = '投资人无任何涉诉、失信记录'
            df_result.loc[0,'SCORE'] = self.score_crit[0]
            df_result.loc[0,'INVESTOR'] = ''
        ...
        Write_Oracle().write_oracle(f'{TableList.BD_RISK_RESULT_CREDIT_CR2.value}',df_result,org_code=self.org_code,alarm=None)
        
        # 整理预警明细数据，并写入数据库
        RISK_ALARM = df_result[df_result['RISK_LABEL'] != '投资人无任何涉诉、失信记录'].groupby(['RISK_LABEL'], as_index=False)['ID'].count()
        ...
        RISK_ALARM['CHILD_MODEL_CODE'] = 'CR2'
        ...
        if RISK_ALARM.empty:
            ...
            Write_Oracle().write_oracle(f'{TableList.BD_RISK_ALARM_ITEM.value}',RISK_ALARM, org_code = self.org_code, alarm = ['CREDIT','CR2'])


    def run_model_cr2(self):
        try:
            self.model_cr2()
            exec_status = 1
        except:
...
if __name__ == '__main__':
    ...
    # child_task_id = 'c_cr2_2'
    org_code, param_json, base_time = read_log_table(child_task_id)
    CreditModelCr2(org_code, params=param_json, base_time = base_time, child_task_id=child_task_id).run_model_cr2()