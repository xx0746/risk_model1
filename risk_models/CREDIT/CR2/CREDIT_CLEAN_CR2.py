...
class CreditCleanCr2(object):
    ...
    def clean_cr2(self):
        ...
        p2=f"""select Corp_name,investor from {TableList.MX_investor.value}"""
        investor_list = Read_Oracle().read_oracle(sql= p2, database = 'dbods')
        corp_investor = pd.merge(corp_list,investor_list,how='inner',on=['CORP_NAME'])
        corp_investor = corp_investor.drop_duplicates(subset=['CORP_NAME','INVESTOR'])

        def role_test_invset(row):
            yg_str = row['YG']
            bg_str = row['BG']
            dsr_str = row['DSR']
            corp_str = row['INVESTOR']
        ...
        t1 = pd.merge(corp_investor,shesu,how='inner',left_on=['INVESTOR'],right_on=['SSQYMC'])
        t1 = t1.drop(columns=['SSQYMC'])
        if t1.shape[0] != 0:
            t1['ROLE'] = t1.apply(role_test_invset,axis=1)
            ...
            t1.columns = ['ORG_NAME','ORG_CODE','INVESTOR','CASE_ID','CASE_INFO','CASE_TIME','STATUS','ROLE','EVENT_TYPE']

        t2 = pd.merge(corp_investor,shixin,how='inner',left_on=['INVESTOR'],right_on=['SSQYMC'])
        ...
        if t2.shape[0] != 0:
            ...
            t2.columns = ['ORG_NAME','ORG_CODE','INVESTOR','CASE_ID','CASE_INFO','STATUS','CASE_TIME']
            t2['EVENT_TYPE'] = '失信被执行'
            t2['ROLE'] = '被执行人'
        ...
        if t1.shape[0] != 0:
            ...
            df_result = pd.DataFrame(columns = ['ORG_NAME','ORG_CODE','INVESTOR','CASE_ID','CASE_INFO','CASE_TIME','STATUS','ROLE','EVENT_TYPE'])
            df_result = df_result.append({'ORG_NAME':org_name,'ORG_CODE':self.org_code,'INVESTOR':'','CASE_ID':'' ,'CASE_INFO':'' ,'CASE_TIME':None ,'STATUS':'' ,'ROLE':'' ,'EVENT_TYPE':'' },ignore_index = True)
        ...
        Write_Oracle().write_oracle(f'{TableList.BD_RISK_DETAIL_CREDIT_CR2.value}',df_result,org_code=self.org_code,alarm=None)


    def run_clean_cr2(self):
        try:
            self.clean_cr2()
            exec_status = 1
        except:
...
if __name__ == '__main__':
    ...
    # child_task_id = 'c_cr2_1'
    org_code, param_json, base_time = read_log_table(child_task_id)
    CreditCleanCr2(org_code, params=param_json, base_time = base_time, child_task_id=child_task_id).run_clean_cr2()