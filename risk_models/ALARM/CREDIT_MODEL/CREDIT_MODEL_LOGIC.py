...
RES_TBL = 'BD_RISK_CROSS_TRADE_RESULT_CREDIT_SCORE_TBLV2'
USER_NAME = "DW_CUS_RC"
PASSWORD = "easipass"
ADDRESS = "192.168.130.225:1521/pdbcusdev"

class CREDIT_MODEL_Logic(object):
    def __init__(self, org_code, base_time, child_task_id):
        ...
        self.columns = ["W1_CUSTOM_WEIGHT", "W2_TAX_CREDIT_WEIGHT", "W3_ADMINISTRATION_WEIGHT", "W4_CRIMINAL_WEIGHT", "W5_LOST_CREDIT_WEIGHT", "W6_EXCUTED_WEIGHT", "W7_NOT_OPERATION_WEIGHT", "W8_SOCIAL_SECURE_WEIGHT", "W9_CATCH_WEIGHT"]
        self.weights = [0.5, 0.5, 0.2, 0.4, 0.2, 0.2, 0.4, 0.2, 0.4]
        self.total_columns = ["W1_CREDIT_RISK_WEIGHT", "W2_ENTERPRISE_RISK_WEIGHT", "W3_OTHER_RISK_WEIGHT"]

        self.total_weights = [0.4, 0.4, 0.2]
    ...
    def logic_layer(self):
        sql_text = f'''
            select CREDIT_CODE, ENTRY_ID, D_DATE, I_E_FLAG, DEAL_TYPE,
            ...
            START_FROM_NOW, 
            TAX_CREDIT 
            FROM DW_CUS_RC.BD_RISK_CROSS_TRADE_RESULT_CREDIT_SCORE_TBL_CLEANV2
            WHERE ISCURRENT = 1
            AND CAL_TYPE = 1
                    '''
        ...
        if SupplyChain_df is not None and len(SupplyChain_df) != 0:
            ...
            if res_df is not None and len(res_df) != 0:
                ...
                print(res_df)
                # 写入数据库
                Write_Oracle_Alarm().write_oracle(RES_TBL, res_df, org_code=None, alarm=None, iscurrent=False)

            if zero_df is not None and len(zero_df) != 0:
                zero_df['TOTAL_SCORE'] = 0
                zero_df['ID'] = 0
                Write_Oracle_Alarm().write_oracle(RES_TBL, zero_df, org_code=None, alarm=None, iscurrent=False)
...
if __name__ == '__main__':
    ...
    if params_global.is_test:
        child_task_id = 'CREDIT_MODEL_LOGIC'
    else:
        child_task_id = sys.argv[1]
    # org_code, param_json, base_time = read_log_table(child_task_id)
    CREDIT_MODEL_Logic(None, None, child_task_id).run_logic_layer()