...
class StockCleanSt1(object):
    ...
    def clean_st1(self):
        ...
        STOCK_OPENING_HEAD = Read_Oracle().read_oracle(sql=""" select * from {} where iscurrent = 1 and CREDIT_CODE = '{}' """.format(_name_OPENING_INVENTORY, self.org_code), database='dbods')
        ...
        # 转化为datetime类型
        # start_time = (start_time - np.datetime64(0, 's')) / np.timedelta64(1, 's')
        ...
        # 汇总期初库存表
        STOCK_OPENING_DETAIL = STOCK_OPENING_DETAIL.groupby(['WAREHOUSE_NO', 'STORAGE_NO', 'GDS_MTNO'])[
            'DCL_QTY'].sum().reset_index()
        STOCK_OPENING_DETAIL = STOCK_OPENING_DETAIL.rename(
            columns={'WAREHOUSE_NO': 'WH_NO', 'STORAGE_NO': 'WH_LOC', 'GDS_MTNO': 'COP_G_NO'})

        # 读取出入库表（限制出入库类型）,并根据输入企业信用代码筛选出入库表
        STOCK_BILL = Read_Oracle().read_oracle(sql=""" select STOCK_BILL_TYPE, ACTRUAL_STOCK_DATE, WH_NO, WH_LOC, COP_G_NO, QTY from {} where CAPXACTION != 'D' and business_type in ('2','3','4') 
                                                     and ORG_CODE = '{}' and QTY is not null""".format(
            _name_WAREHOUSE_STOCK_BILL, self.org_code), database='dbods')
     
        # 根据期初和期末时间，筛选出入库表
        STOCK_BILL = STOCK_BILL[(STOCK_BILL['ACTRUAL_STOCK_DATE'] >= start_time) & (STOCK_BILL['ACTRUAL_STOCK_DATE'] <= end_time)]
        ...
        # 汇总出入库表
        STOCK_BILL = STOCK_BILL.groupby(['WH_NO', 'WH_LOC', 'COP_G_NO'])['QTY'].sum().reset_index()
        ...
        STOCK_END_DETAIL = STOCK_END_DETAIL[
            ['ID', 'ORG_CODE', 'STARTDT', 'ENDDT', 'WH_NO', 'WH_LOC', 'COP_G_NO', 'QTY_BEFORE', 'QTY_CHANGE',
             'QTY_AFTER', 'CHECK_TIME', 'CUSTOMER_CODE']]

        # 写入数据库
        Write_Oracle().write_oracle('BD_RISK_DETAIL_STOCK_ST1', STOCK_END_DETAIL, org_code=self.org_code, alarm=None)

    def run_clean_st1(self):
        try:
            self.clean_st1()
            exec_status = 1
...
if __name__ == '__main__':
    ...
    if params_global.is_test:
        child_task_id = '2271842168e8452a82be505de685ae13'
    else:
        child_task_id = sys.argv[1]
    org_code, param_json, base_time = read_log_table(child_task_id)
    StockCleanSt1(org_code, base_time, child_task_id).run_clean_st1()